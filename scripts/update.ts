...
const sendGetRequest = async (url: string): Promise<any> => {
    const res = await fetch(url, {
        headers: { Authorization: `Token ${await config.pluggableDB.getTokenC()}`}
    })
...
const sendCredentialsRequest = async (url: string, tokenB: string): Promise<any> => {
    const res = await fetch(url, {
        method: "POST",
        headers: {
            "Authorization": `Token ${await config.pluggableDB.getTokenC()}`,
...
register()
    .then((credentials) => console.log(`Registration to ${config.registration.versionsURL} updated. Got TOKEN_C = ${credentials.token}`))
